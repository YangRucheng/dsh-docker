...
- **手写声明（自定义 OpenAI 兼容网关等）或目录中未声明推理能力的模型**：显示完整的通用等级梯子 `Off / Minimal / Low / Medium / High / Extra High / Max`，选择后按原样发送给供应商（如 `reasoning_effort` 等 wire 参数）；不选择时保持发送行为不变（跟随供应商默认）。
